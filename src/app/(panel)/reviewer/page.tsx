    ...
    try {
        ...
        return (
            <section className="space-y-6 pb-20">
                ...
                {/* Role Banner */}
                <Card className="bg-emerald-50/50 dark:bg-emerald-500/5 border-emerald-100 dark:border-emerald-500/10">
                    <CardContent className="p-6 sm:p-8">
                        <div className="flex flex-col md:flex-row items-center gap-6">
                            ...
                            <div className="flex-1 text-center md:text-left space-y-2">
                                <Badge variant="outline" className="text-[10px] 2xl:text-xs font-bold tracking-widest uppercase border-emerald-200 dark:border-emerald-500/20 text-emerald-600">Peer Reviewer Protocol</Badge>
                                <h2 className=" font-black text-foreground tracking-wider uppercase">Active Mission: Ensure Scientific Integrity</h2>
                                <p className="text-sm 2xl:text-base text-muted-foreground max-w-2xl 2xl:max-w-4xl font-medium opacity-80 leading-relaxed transition-all duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 2xl:gap-8 transition-all duration-500">
                    {stats.map((stat) => (
                        <Card key={stat.label} className="border-border/50 shadow-sm bg-background/50 backdrop-blur-sm group hover:border-primary/20 transition-all">
                            <CardContent className="p-6 flex items-center justify-between">
                                <div className="space-y-1 2xl:space-y-2">
                                    ...
                                    <h3 className=" font-black text-foreground">{stat.value}</h3>
                                </div>
                                <div className="w-12 h-12 rounded-xl bg-muted/50 flex items-center justify-center transition-transform group-hover:scale-110">
                                    {stat.icon}
                                </div>
                            </CardContent>
                        </Card>
                ...
                <Tabs defaultValue="assignments" className="space-y-4">
                    <TabsList className="bg-muted/50 p-1 rounded-xl w-full flex flex-wrap sm:inline-flex h-auto gap-1 2xl:gap-2">
                        <TabsTrigger value="assignments" className="flex-1 sm:flex-none rounded-lg px-4 py-2 sm:px-8 2xl:px-12 2xl:py-3 text-[10px] sm:text-xs 2xl:text-sm font-bold uppercase tracking-widest transition-all whitespace-nowrap">Assigned Works</TabsTrigger>
                        <TabsTrigger value="my-papers" className="flex-1 sm:flex-none rounded-lg px-4 py-2 sm:px-8 2xl:px-12 2xl:py-3 text-[10px] sm:text-xs 2xl:text-sm font-bold uppercase tracking-widest transition-all whitespace-nowrap">Personal Research</TabsTrigger>
                    </TabsList>
                                        ...
                                        {assignedRows.map((sub: any) => (
                                            ...
                                            >
                                                <div className="flex items-center gap-4 min-w-0">
                                                    <div className="w-12 h-10 rounded-lg bg-muted flex items-center justify-center font-mono font-bold text-[10px] text-muted-foreground border border-border shrink-0 group-hover:bg-background transition-colors">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-[10px] 2xl:text-xs text-muted-foreground font-medium uppercase opacity-70">
                                                            By {sub.author_name} • {new Date(sub.assigned_at).toLocaleDateString()}
                                                        </p>
                                                    </div>
                                                </div>
                                                <Badge className={`border-none text-[10px] font-bold uppercase py-0.5 px-2 bg-primary/10 text-primary`}>
                                ...
                                <Card key={paper.id} className="border-border/50 shadow-sm bg-background hover:shadow-md transition-all group overflow-hidden rounded-xl">
                                    <div className="p-6 space-y-4">
                                        ...
                                        <div className="flex items-center justify-between text-[10px] text-muted-foreground pt-4 border-t border-border/10">
                                            ...
                                            <Button asChild variant="ghost" size="sm" className="h-7 px-3 text-primary hover:bg-primary/5 rounded-md font-bold uppercase text-[9px] cursor-pointer">
