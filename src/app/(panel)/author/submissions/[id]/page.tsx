...
import { FileText, Calendar, User, Eye, Download, History, MessageSquare, AlertTriangle } from "lucide-react";
...
export default async function AuthorSubmissionDetailsPage({ params }: { params: { id: string } }) {
    ...
    const sub = await getAuthorSubmission(submissionId);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3 text-sm text-primary/40 font-bold uppercase tracking-widest leading-none">
                        ...
                        <span>Submitted on {format(new Date(sub.submitted_at), "MMMM dd, yyyy")}</span>
                    </div>
                </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <Card className="border-primary/10 shadow-xl shadow-primary/5 overflow-hidden">
                        <CardHeader className="bg-primary/[0.01] border-b border-primary/5">
                        ...
                        <CardContent className="pt-6 space-y-6">
                            ...
                            <div className="pt-6 border-t border-primary/5 grid grid-cols-2 gap-6">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-sm font-bold text-primary">v{sub.version_number}</p>
                                </div>
                                <div className="space-y-1 text-right">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-primary/30">Last Updated</span>
                                    <p className="text-sm font-bold text-primary">{format(new Date(sub.updated_at), "MMM dd, yyyy")}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    {sub.reviewComments && sub.reviewComments.length > 0 && (
                        <Card className="border-orange-100 shadow-xl shadow-orange-500/5 overflow-hidden">
                            <CardHeader className="bg-orange-50/50 border-b border-orange-100">
                                <div className="flex items-center gap-3">
                                    <MessageSquare className="w-5 h-5 text-orange-600" />
                                    <CardTitle className="text-xl font-black text-orange-900 uppercase tracking-tight">Editorial Feedback</CardTitle>
                                </div>
                                <CardDescription className="text-orange-700/60 font-bold uppercase tracking-widest text-[10px]">Please address these comments in your revision.</CardDescription>
                            </CardHeader>
                            <CardContent className="pt-6 space-y-6">
                                {sub.reviewComments.map((comment: any, idx: number) => (
                                    <div key={idx} className="p-6 rounded-2xl bg-orange-50/30 border border-orange-100 space-y-4">
                                        <div className="flex items-center justify-between">
                                            <Badge className="bg-orange-600 text-white border-0 text-[10px] font-black tracking-widest rounded-lg">ROUND {comment.review_round}</Badge>
                                            <span className="text-[10px] font-black uppercase tracking-widest text-orange-400">Received {format(new Date(comment.submitted_at), "MMM dd")}</span>
                                        </div>
                                        <div className="text-sm text-orange-900 leading-relaxed font-medium whitespace-pre-wrap">
                                            {comment.comments_to_author}
                                        </div>
                                    </div>
                                ))}
                            </CardContent>
                ...
                <div className="space-y-8">
                        ...
                        <CardContent className="pt-6 space-y-3">
                            {sub.files.map((file: any) => (
                                <Link 
                                    ...
                                    href={file.file_url} 
                                    target="_blank"
                                    className="flex items-center justify-between p-4 rounded-xl border border-primary/5 hover:bg-primary/[0.02] hover:border-secondary transition-all group"
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-xs font-black text-primary/60 truncate max-w-[120px]">{file.original_name}</p>
                                            <p className="text-[10px] font-bold text-primary/30 uppercase tracking-widest">{file.file_type.replace('_', ' ')}</p>
                                        </div>
                                    </div>
                                    <Badge variant="outline" className="text-[10px] border-primary/5 text-primary/40">{(file.file_size / 1024 / 1024).toFixed(1)}MB</Badge>
                                </Link>
                            ))}
                        </CardContent>
                    </Card>
                        ...
                        <CardContent className="pt-6 space-y-4">
                            {sub.authors.map((author: any, idx: number) => (
                                <div key={idx} className="flex items-start gap-3">
                                    <div className="w-8 h-8 rounded-full bg-primary/5 flex items-center justify-center flex-shrink-0 text-primary/40 font-black text-[10px]">
                                        {idx + 1}
                                    </div>
                    ...
                    {sub.publication && (
                        <Link 
                            href={sub.publication.final_pdf_url}
                            target="_blank"
                        ...
                        >
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-xs text-white/60 font-black uppercase tracking-widest">Volume {sub.publication.volume_number}, Issue {sub.publication.issue_number}</p>
                            </div>
