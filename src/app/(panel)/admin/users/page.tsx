...
export default function UserManagement() {
    ...
    if (loading) return <div className="p-20 text-center font-black text-muted-foreground  tracking-widest text-xs animate-pulse ">Scanning Directory...</div>;
        ...
        <section className="space-y-6">
            ...
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-primary/5 pb-8 2xl:pb-16 transition-all duration-500">
                ...
                <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
                    <DialogTrigger asChild>
                        <Button className="h-11 2xl:h-14 px-5 2xl:px-8 gap-2.5 bg-primary text-white dark:text-slate-900 font-black text-[9px] sm:text-[10px] xl:text-[11px] 2xl:text-xs tracking-widest rounded-xl shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer">
                            <UserPlus className="w-5 h-5 2xl:w-6 2xl:h-6" /> Add Staff Member
                        </Button>
                    </DialogTrigger>
                    <DialogContent className="sm:max-w-md rounded-xl p-6 bg-card border-border/50">
                        <DialogHeader>
                            <DialogTitle className="text-2xl font-black text-foreground tracking-wider">Invite Staff Member</DialogTitle>
                        ...
                        <form action={async (formData) => {
                            ...
                            <div className="space-y-2">
                                <label htmlFor="staff-fullName" className="text-[9px] sm:text-[10px] xl:text-[11px] 2xl:text-xs font-black text-muted-foreground tracking-widest px-1 uppercase">Full Name</label>
                                <Input id="staff-fullName" name="fullName" required className="h-12 bg-muted/50 border-none focus-visible:ring-1 focus-visible:ring-primary/30 font-bold text-sm rounded-xl" placeholder="Dr. Jane Smith" />
                            </div>
                            <div className="space-y-2">
                                <label htmlFor="staff-email" className="text-xs font-black text-muted-foreground tracking-widest px-1 uppercase">Email Address</label>
                                <Input id="staff-email" name="email" type="email" required className="h-12 bg-muted/50 border-none focus-visible:ring-1 focus-visible:ring-primary/30 font-bold text-sm rounded-xl" placeholder="jane@ijitest.com" />
                            </div>
                            <div className="space-y-2">
                                <label htmlFor="staff-role" className="text-xs font-black text-muted-foreground tracking-widest px-1 uppercase">User Role</label>
                                <select id="staff-role" name="role" required className="flex h-12 w-full rounded-xl bg-muted/50 px-3 py-1 text-sm font-bold transition-colors outline-none border-none ring-offset-background placeholder:text-muted-foreground focus:ring-1 focus:ring-primary/30 disabled:cursor-not-allowed disabled:opacity-50 text-foreground">
                            ...
                            <DialogFooter className="pt-4">
                                <Button type="submit" className="w-full h-12 font-black text-xs tracking-widest shadow-xl shadow-primary/20 rounded-xl bg-primary text-white dark:text-black dark:text-slate-900 hover:bg-primary/90 cursor-pointer">
                                    Send Invitation
                                </Button>
                            </DialogFooter>
                        </form>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 2xl:gap-8 transition-all duration-500">
                {users.length === 0 ? (
                    <div className="col-span-full py-20 bg-muted/20 border-2 border-dashed border-border/50 rounded-xl flex flex-col items-center justify-center text-center">
                        ...
                        <h3 className=" font-black text-muted-foreground tracking-wider">No Staff Found</h3>
            ...
            <Dialog open={!!userToDelete} onOpenChange={(open) => !open && setUserToDelete(null)}>
                <DialogContent className="sm:max-w-md rounded-xl p-6">
                    <DialogHeader className="space-y-3">
                        ...
                        <DialogTitle className="text-xl font-black text-foreground tracking-wider">Revoke Staff Access?</DialogTitle>
                        <DialogDescription className="text-sm font-medium text-muted-foreground leading-relaxed">
                            You are about to revoke system access for <span className="text-foreground font-bold">{userToDelete?.full_name}</span>. This action will immediately invalidate their credentials and remove their administrative permissions.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="bg-muted/30 p-5 rounded-xl border border-border/50 mb-2">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="min-w-0">
                                <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Account email</p>
                                <p className="text-sm font-bold text-foreground truncate">{userToDelete?.email}</p>
                            </div>
                        </div>
                    </div>
                    <DialogFooter className="flex flex-col sm:flex-row gap-3 mt-4">
                        <Button
                            ...
                            disabled={isDeleting}
                            className="flex-1 h-11 text-[11px] font-black uppercase tracking-widest rounded-xl"
                        >
                            Maintain Access
                        </Button>
                        <Button
                            ...
                            disabled={isDeleting}
                            className="flex-1 h-11 text-[11px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-destructive/20"
                        >
            ...
            <div className="mt-12 space-y-4">
                <div className="flex items-center gap-3 border-b border-border pb-3">
                    <h2 className=" font-black text-foreground tracking-widest uppercase transition-all duration-500">Role Reference Guide</h2>
                        ...
                        <Card key={guide.role} className="border-border/50 shadow-sm">
                            <CardContent className="p-6">
                                <div className="flex flex-col gap-3 mb-5">
                                    ...
                                    <div>
                                        <h3 className="font-black text-foreground tracking-wider ">{guide.role} <span className="text-[11px] text-muted-foreground font-semibold uppercase block mt-1">{guide.title}</span></h3>
                                    </div>
                                </div>
                                <p className="text-sm font-bold text-muted-foreground leading-relaxed mb-6">{guide.desc}</p>
                                <div className="space-y-2">
                                    <p className="text-[10px] font-black text-muted-foreground/50 tracking-widest mb-3 uppercase">Key Actions</p>
                                    {guide.actions.map((action, i) => (
                                        <div key={i} className="flex items-center gap-3 text-xs font-bold text-foreground bg-muted/30 px-3 py-2 rounded-xl">
